...
(function () {
    ...
    function isUpgradeOverviewOpen() {
        const headerEl = document.getElementById('headerBar') || document.querySelector('.hud-window__page');
        if (!headerEl) return false;
        const titleText = headerEl.querySelector('.text-h1, h1')?.textContent || '';
        return titleText.includes('Upgrade-Übersicht');
    }
    ...
    function findExpandButton() {
        const expandBtns = Array.from(document.querySelectorAll('button.variant--normal')).filter(btn => {
            ...
            return textContent.includes('ausbauen') || textContent.includes('upgraden') || textContent.includes('parkplatz');
        });
    ...
    function injectScanButton() {
        ...
        const headerContainer = document.querySelector('#headerBar .gap-md') ||
            document.querySelector('img[src*="improvement_arrow"]')?.closest('.flex.flex-nowrap.items-center')?.querySelector('.gap-md') ||
            document.querySelector('.text-h1')?.closest('.flex.items-center')?.querySelector('.gap-md');
    ...
    // -----------------------------------------------------------------------
    function init() {
        console.log('[LEA Auto Upgrade] Initialisiert v1.1.16');
